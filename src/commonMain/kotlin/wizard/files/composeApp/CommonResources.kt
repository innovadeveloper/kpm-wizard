...
class CommonCounterScreenJvm(info: ProjectInfo, isOnlyJvm : Boolean = false) : ProjectFile {
    //    override val path = "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/ui/screen/counter/CounterScreen.kt"
    override val path = if(isOnlyJvm) "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/ui/screen/counter/CounterScreen.kt" else "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/ui/screen/counter/CounterScreen.kt"
    override val content = """
package ${info.packageId}.ui.screen.counter
...
import androidx.compose.ui.unit.dp
import ${info.packageId}.di.qualifiers.ServiceQualifier
import ${if(isOnlyJvm) """com.abexa.shared.screen.AbstractScreen""".trimIndent() else """${info.packageId}.ui.screen.AbstractScreen""".trimIndent()}
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
...
class CommonCounterViewModelJvm(info: ProjectInfo, isOnlyJvm : Boolean = false) : ProjectFile {
    //    override val path = "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/ui/screen/counter/CounterViewModel.kt"
    override val path = if(isOnlyJvm) "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/ui/screen/counter/CounterViewModel.kt" else "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/ui/screen/counter/CounterViewModel.kt"
    override val content = """
package ${info.packageId}.ui.screen.counter
...
import ${info.packageId}.infrastructure.extensions.safeCallAsPair
//import ${info.packageId}.ui.screen.AbstractViewModel
import ${if(isOnlyJvm) """com.abexa.shared.screen.AbstractViewModel""".trimIndent() else """${info.packageId}.ui.screen.IViewModel""".trimIndent()}
import ${if(isOnlyJvm) """com.abexa.shared.screen.IViewModel""".trimIndent() else """${info.packageId}.ui.screen.IViewModel""".trimIndent()}
//import ${info.packageId}.ui.screen.IViewModel
import kotlinx.coroutines.flow.MutableStateFlow
import java.util.Base64
...
class CommonDBDriverJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/db/DBDriver.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/db/DBDriver.kt"
    override val content = """
package ${info.packageId}.data.db
...
class CommonDBRepositoryJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/db/repositories/OrderRepository.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/db/repositories/OrderRepository.kt"
...
class CommonOrdersSQLDelightJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/sqldelight/${info.packagePath}/orders.sq"
    else
        "${info.moduleName}/src/commonMain/sqldelight/${info.packagePath}/orders.sq"
...
class CommonUsersSQLDelightJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/sqldelight/${info.packagePath}/users.sq"
    else
        "${info.moduleName}/src/commonMain/sqldelight/${info.packagePath}/users.sq"
...
class CommonSchemasSQLDelightJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/sqldelight/${info.packagePath}/schemas.sq"
    else
        "${info.moduleName}/src/commonMain/sqldelight/${info.packagePath}/schemas.sq"
    override val content = """
CREATE TABLE users (
...
class CommonNetworkKoinQualifierJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/di/qualifiers/NetworkQualifier.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/di/qualifiers/NetworkQualifier.kt"
...
class CommonServiceKoinQualifierJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/di/qualifiers/ServiceQualifier.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/di/qualifiers/ServiceQualifier.kt"
...
class CommonKoinInitJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/di/KoinInit.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/di/KoinInit.kt"
...
class CommonKoinServiceJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/di/ServiceModule.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/di/ServiceModule.kt"
...
// com.abexa.kmp.data.network.providers
class CommonHTTPGenericProviderJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/network/providers/HTTPGenericProvider.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/network/providers/HTTPGenericProvider.kt"
...
fun createAnotherServerAPI2(baseUrl : String, appParamsDTO: AppParamsDTO): AnotherServerAPI{
    return HTTPGenericProvider(baseUrl, appParamsDTO).create().createAnotherServerAPI()
}
fun createIdentityServerAPI2(baseUrl : String, appParamsDTO: AppParamsDTO): IdentityServerAPI{
...
class CommonPreferencesSPFJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/spf/Preferences.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/spf/Preferences.kt"
...
class CommonNetworkAnotherServerAPIJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/network/AnotherServerAPI.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/network/AnotherServerAPI.kt"
...
class CommonNetworkIdentityServerAPIJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/data/network/IdentityServerAPI.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/data/network/IdentityServerAPI.kt"
    override val content = """
package ${info.packageId}.data.network
...
class CommonConstantsJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm) "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/domain/constants/Constants.kt"
    else "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/domain/constants/Constants.kt"
...
class CommonRequestTokenDTOJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/domain/dto/request/TokenRequest.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/domain/dto/request/TokenRequest.kt"
    override val content = """
package ${info.packageId}.domain.dto.request
...
class CommonAccessTokenDTOJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/domain/dto/AccessTokenDTO.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/domain/dto/AccessTokenDTO.kt"
...
class CommonAppParamsDTOJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/domain/dto/AppParamsDTO.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/domain/dto/AppParamsDTO.kt"
...
class CommonExtensionsJvm(info: ProjectInfo, isOnlyJvm: Boolean = false) : ProjectFile {
    override val path = if (isOnlyJvm)
        "${info.moduleName}/src/desktopMain/kotlin/${info.packagePath}/infrastructure/extensions/Extensions.kt"
    else
        "${info.moduleName}/src/commonMain/kotlin/${info.packagePath}/infrastructure/extensions/Extensions.kt"
